...
def found_affinity_unseen_paris(seen_pairs,unseen_pairs,dis):
    if dis == 'cosine':
        similarity = seen_pairs@unseen_pairs.T
        _,pairs_index = torch.max(similarity,dim=1)
        return pairs_index
    elif dis == 'complex':
        similarity = complex_similarity(seen_pairs,unseen_pairs)
        _, pairs_index = torch.max(similarity, dim=1)
        return pairs_index
...
def equivalence_class(input):
    model_lenth = ((input[:,0,:]**2+input[:,1,:]**2)**0.5).view(input.shape[0],-1)
    values,index = torch.max(model_lenth,dim=-1)
    values = values.view(input.shape[0],1,1)
    output = input/values

    row_indices = torch.arange(input.shape[0]).view(-1, 1).to(device)
    col_indices = index.view(-1, 1)
    index_tensor = torch.cat([row_indices, col_indices], dim=1)

    output[index_tensor[:, 0], :, index_tensor[:, 1]] = 0

    return output

def compute_real_unreal_similarity(x1,x2):
    x1_e = x1
    x2_e = x2
    real = torch.mm(x1_e[:,0,:],x2_e[:,0,:].T)-torch.mm(x1_e[:,1,:],-x2_e[:,1,:].T)
    unreal = torch.mm(x1_e[:,1,:],x2_e[:,0,:].T)+torch.mm(x1_e[:,0,:],-x2_e[:,1,:].T)
    return real,unreal

def complex_similarity(x1,x2,mode='minos'):
    real_upper, unreal_upper = compute_real_unreal_similarity(x1, x2)
    real_x1,unreal_x1 = compute_real_unreal_similarity(x1,x1)
    real_x2,unreal_x2 = compute_real_unreal_similarity(x2,x2)
    diag_real_x1 = torch.diag(real_x1).view(-1,1)
    diag_real_x2 = torch.diag(real_x2).view(1,-1)
    supp = torch.mm(diag_real_x1, diag_real_x2)
    if mode == 'minos':
        upper = real_upper**2-unreal_upper**2
        output = upper/(supp+1e-20)
    elif mode == 'single':
        upper = real_upper ** 2
        output = upper
    else:
